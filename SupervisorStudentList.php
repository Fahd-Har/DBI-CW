...
$supervisorId = $_SESSION['assessor_id'];

$stmt = $conn->prepare("
  SELECT
    i.InternshipID, s.StudentID, s.Name AS StudentName, s.Programme, c.CompanyName,
    sup_a.AssessmentID AS SupAssessmentID,
    sup_a.UndertakingTaskOrProject, sup_a.HealthSafetyWorkplace, sup_a.ConnectivityTheoreticalKnowledge,
    sup_a.PresentationWrittenDocument, sup_a.ClarityLanguageIllustration, sup_a.LifelongLearningActivities,
    sup_a.ProjectManagement, sup_a.TimeManagement, sup_a.Comments AS SupComments,
    (
  ...
  LEFT JOIN assessment sup_a ON sup_a.InternshipID = i.InternshipID AND sup_a.SupervisorID = ?
...
while ($r = $result->fetch_assoc()) {
    $supTotal = null;
    if ($r['SupAssessmentID']) {
        $supTotal = (int)$r['UndertakingTaskOrProject'] + (int)$r['HealthSafetyWorkplace'] + (int)$r['ConnectivityTheoreticalKnowledge']
    ...
    $r['SupervisorTotal'] = $supTotal;
    $r['FinalMark'] = ($supTotal !== null && $r['LecturerTotal'] !== null)
        ? round(($supTotal + $r['LecturerTotal']) / 2, 1)
        : null;
    $rows[] = $r;
}
...
<aside class="sidebar">
  ...
  <ul class="sidebar-nav">
    <li><a href="SupervisorStudentList.php" class="active"><img src="users.png" alt="" class="icon"/> Student List</a></li>
  </ul>
    ...
    <table>
      ...
      <tbody id="tableBody">
        ...
        <tr data-id="<?= $r['StudentID'] ?>"
            ...
            data-comments="<?= htmlspecialchars($r['SupComments'] ?? '') ?>">
          <td><?= $r['StudentID'] ?></td>
          ...
          <td><button class="btn-edit" onclick="openMarksModal(this)"><?= $r['SupervisorTotal'] !== null ? 'Edit Marks' : 'Enter Marks' ?></button></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
...
<div class="modal-overlay" id="modalOverlay" onclick="closeModalOutside(event)">
  <div class="modal">
    ...
    <form method="POST" action="supervisor_assessment_actions.php">
