...
if ($action === 'delete') {
    $id = intval($_POST['lecturer_id'] ?? 0);
    if ($id <= 0) {
        header("Location: User_Management_Lecturer.php?error=Invalid lecturer ID");
        exit;
    }
    ...
    try {
        ...
        // 2. delete this lecturer's assessment rows
        $d1 = $conn->prepare("DELETE FROM assessment WHERE LecturerID = ?");
        $d1->bind_param("i", $id);
        $d1->execute();

        // 3. detach internships pointing at this lecturer (column is NULLABLE)
        $d2 = $conn->prepare("UPDATE internship SET LecturerID = NULL WHERE LecturerID = ?");
        $d2->bind_param("i", $id);
        $d2->execute();

        // 4. delete the lecturer row
