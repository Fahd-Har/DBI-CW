...
// Safety: ensure this internship actually belongs to this supervisor
$chk = $conn->prepare("SELECT InternshipID FROM internship WHERE InternshipID = ? AND SupervisorID = ?");
$chk->bind_param("ii", $internshipId, $supervisorId);
$chk->execute();
if (!$chk->get_result()->fetch_assoc()) {
    header("Location: SupervisorStudentList.php?error=Not authorised for this internship");
    exit;
}
...
try {
    $q = $conn->prepare("SELECT AssessmentID FROM assessment WHERE InternshipID = ? AND SupervisorID = ? LIMIT 1");
    $q->bind_param("ii", $internshipId, $supervisorId);
    $q->execute();
    ...
    header("Location: SupervisorStudentList.php?error=" . urlencode("Database error: " . $e->getMessage()));
}
exit;
?>
